...
const KanbanBoard = () => {
  // Load initial state from local storage, or use defaults if nothing is saved
  ...
  // Save grouping and sorting in local storage whenever they change
  useEffect(() => {
          ...
          return (
            <div key={groupKey} className="kanban-column">
              <div className="header-container">
                ...
                {grouping === 'user' && (
                  <>
                    <div className="initials-avatar" style={{ backgroundColor: "#2196F3" }}>
                      {user ? `${user.name[0]}${user.name[1]}` : "NA"}
                    </div>
                    <h3 className="user-name">{groupKey}</h3>
                  </>
                )}
